...
import { EffectsTimelineIntegration, createDefaultEffectTracks } from '../src/effects/effects-timeline-integration'
...
describe('EffectsTimelineIntegration', () => {
  let integration: EffectsTimelineIntegration

  beforeEach(() => {
    integration = new EffectsTimelineIntegration(mockEffectsSystem, mockTimeline)
    vi.clearAllMocks()
  })
  ...
  describe('Initialization', () => {
    it('should initialize successfully', async () => {
      ...
      expect(mockTimeline.addEventListener).toHaveBeenCalledWith('timeChanged', expect.any(Function))
      expect(mockTimeline.addEventListener).toHaveBeenCalledWith('trackAdded', expect.any(Function))
      expect(mockTimeline.addEventListener).toHaveBeenCalledWith('trackRemoved', expect.any(Function))
      expect(mockTimeline.addEventListener).toHaveBeenCalledWith('keyframeAdded', expect.any(Function))
      expect(mockTimeline.addEventListener).toHaveBeenCalledWith('keyframeRemoved', expect.any(Function))
    })
  ...
  describe('Effect Parameter Tracks', () => {
    it('should create effect parameter track successfully', async () => {
      ...
      const result = integration.createEffectParameterTrack('effect-1', 'intensity', 1.0)
    ...
    it('should handle creating track for non-existent effect', async () => {
      ...
      const result = integration.createEffectParameterTrack('nonexistent', 'intensity', 1.0)
    ...
    it('should create multiple parameter tracks for same effect', async () => {
      ...
      const result1 = integration.createEffectParameterTrack('effect-1', 'intensity', 1.0)
      const result2 = integration.createEffectParameterTrack('effect-1', 'radius', 10)
  ...
  describe('Keyframe Management', () => {
    ...
    it('should add keyframe to effect parameter successfully', () => {
      const result = integration.addEffectKeyframe('effect-1', 'intensity', 2.0, 0.5)
    ...
    it('should handle adding keyframe to non-existent effect', () => {
      const result = integration.addEffectKeyframe('nonexistent', 'intensity', 2.0, 0.5)
    ...
    it('should handle adding keyframe to non-existent parameter', () => {
      const result = integration.addEffectKeyframe('effect-1', 'nonexistent', 2.0, 0.5)
    ...
    it('should remove keyframe from effect parameter successfully', () => {
      ...
      const addResult = integration.addEffectKeyframe('effect-1', 'intensity', 2.0, 0.5)
      expect(addResult.success).toBe(true)
      
      // Then remove it
      const removeResult = integration.removeEffectKeyframe('effect-1', 'intensity', addResult.data!.id)
    ...
    it('should handle removing keyframe from non-existent effect', () => {
      const result = integration.removeEffectKeyframe('nonexistent', 'intensity', 'keyframe-1')
  ...
  describe('Timeline Event Handling', () => {
    ...
    it('should handle time change events', () => {
      ...
      const timeChangeHandler = mockTimeline.addEventListener.mock.calls
        .find(call => call[0] === 'timeChanged')?.[1]
    ...
    it('should handle track added events', () => {
      const trackAddedHandler = mockTimeline.addEventListener.mock.calls
        .find(call => call[0] === 'trackAdded')?.[1]
    ...
    it('should handle track removed events', () => {
      const trackRemovedHandler = mockTimeline.addEventListener.mock.calls
        .find(call => call[0] === 'trackRemoved')?.[1]
  ...
  describe('Interpolation', () => {
    ...
    it('should handle linear interpolation', () => {
      ...
      integration.addEffectKeyframe('effect-1', 'intensity', 0.0, 0.0, InterpolationMode.Linear)
      integration.addEffectKeyframe('effect-1', 'intensity', 2.0, 2.0, InterpolationMode.Linear)
    ...
    it('should handle stepped interpolation', () => {
      ...
      integration.addEffectKeyframe('effect-1', 'intensity', 0.0, 0.0, InterpolationMode.Stepped)
      integration.addEffectKeyframe('effect-1', 'intensity', 2.0, 2.0, InterpolationMode.Stepped)
    ...
    it('should handle smooth interpolation', () => {
      ...
      integration.addEffectKeyframe('effect-1', 'intensity', 0.0, 0.0, InterpolationMode.Smooth)
      integration.addEffectKeyframe('effect-1', 'intensity', 2.0, 2.0, InterpolationMode.Smooth)
...
describe('Default Effect Tracks', () => {
  let integration: EffectsTimelineIntegration
  ...
  it('should create default tracks for glow effect', () => {
    ...
    expect(tracks.some(track => track.parameterName === 'intensity')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'radius')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'threshold')).toBe(true)
  })

  it('should create default tracks for gaussian blur effect', () => {
    const result = createDefaultEffectTracks(integration, 'blur-1', 'gaussianBlur')
  ...
  it('should create default tracks for brightness/contrast effect', () => {
    const result = createDefaultEffectTracks(integration, 'bc-1', 'brightnessContrast')
    
    expect(result.success).toBe(true)
    
    const tracks = integration.getEffectParameterTracks('bc-1')
    expect(tracks).toHaveLength(2)
    expect(tracks.some(track => track.parameterName === 'brightness')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'contrast')).toBe(true)
  })
  ...
  it('should create default tracks for levels effect', () => {
    ...
    expect(tracks.some(track => track.parameterName === 'inputBlack')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'inputWhite')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'gamma')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'outputBlack')).toBe(true)
    expect(tracks.some(track => track.parameterName === 'outputWhite')).toBe(true)
  })

  it('should handle unknown effect type', () => {
    const result = createDefaultEffectTracks(integration, 'unknown-1', 'unknown')
